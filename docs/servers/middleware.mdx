...
<VersionBadge version="4.0.8" />

Called for a `completion/complete` request. The `context.message` carries `ref` (the prompt or resource template being completed), `argument` (its `name` and the partial `value`), and `context` (arguments already resolved).

Completion resolves its reference like `prompts/get` does, so a middleware that hides components from listings (in `on_list_prompts` or `on_list_resource_templates`) does not hide them from completion. To withhold suggestions, handle it here, and return an empty completion rather than raising so the response does not confirm the component exists. `AuthMiddleware` does this for its own checks.

```python
from mcp_types import CompleteResult, Completion, PromptReference

async def on_complete(self, context: MiddlewareContext, call_next):
    ref = context.message.ref
    if isinstance(ref, PromptReference) and ref.name.startswith("internal_"):
