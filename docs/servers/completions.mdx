...
The handler only runs for references the caller could get. A prompt or template hidden by `disable()`, a visibility transform, component auth, or `AuthMiddleware` gets an empty completion, exactly as an unknown reference would, so suggestions never confirm that a hidden component exists. Custom middleware that hides components only from listings should also implement [`on_complete`](/servers/middleware#on_complete).
